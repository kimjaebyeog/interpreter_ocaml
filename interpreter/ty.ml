...
let rec gather_ty_constraints : ty_env -> expr -> ty * ty_constraints
    ...
    | ERLet (f, x, e1, e2) ->
        let a = new_ty_var ()
        in let b = new_ty_var ()
        in let (t1, c1) = gather_ty_constraints ((x,TVar a)::(f, TFun(TVar a,TVar b))::tenv) e1
        in let (t2, c2) = gather_ty_constraints ((f, TFun(TVar a,TVar b))::tenv) e2
        in (t2, (t1,TVar b)::c1@c2)
    | EMRLet (l, e) ->
        (* list of types of function freshly generated *)
        let fun_types : ty_env = List.map (fun (f,_,_) -> (f, TFun (TVar (new_ty_var ()), TVar (new_ty_var ())))) l
        (* constraints gathered from each function bodies (flattened)*)
        in let cntns : ty_constraints = List.concat (List.mapi (fun i (_,x,e) ->
            let (t1,t2) = match snd (List.nth fun_types i) with
                (TFun (t1, t2)) -> (t1,t2)
                | _ -> raise (Ty_error "Should not be able to reach here (in mutual rec let, function does not match function type)")
            in let (t, c) = gather_ty_constraints ((x, t1)::fun_types@tenv) e
            in (t,t2)::c) l)
        in let (t, c) = gather_ty_constraints (fun_types@tenv) e
        in (t, cntns@c)
    | EMatch (e, l) ->
