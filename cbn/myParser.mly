...
comm:
  ...
  | LET REC x=var e=eq_fun_expr DSEMI  { CRLet (x, e) }
  | LET REC x=var e=eq_fun_expr AND l=mutual_rec_fun DSEMI  { CMRLet ((x, e) :: l) }
;
expr:
  | IF e0=expr THEN e1=expr ELSE e2=expr                { EIf (e0,e1,e2) }
  | LET x=var e1=eq_fun_expr IN e2=expr                 { ELet (x ,e1, e2) }
  | LET REC x=var e1=eq_fun_expr IN e2=expr             { ERLet (x, e1, e2) }
  | LET REC x=var e1=eq_fun_expr AND l=mutual_rec_fun IN e2=expr  { EMRLet (((x, e1) :: l), e2) }
  | FUN e=fun_expr                                      { e }
...
mutual_rec_fun:
  | x=var e=eq_fun_expr AND l=mutual_rec_fun { (x, e) :: l }
  | x=var e=eq_fun_expr                      { (x, e) :: [] }
;
